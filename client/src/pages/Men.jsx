...
// ------------------------------------------------------------------
// The Men's Edit - a dedicated, fully animated landing page for the
// men's collection. It owns its own route (/men) and never mixes
// ladies' pieces into its grid. Category filtering is available for
// refinement but the page is locked to gender=men.
// ------------------------------------------------------------------
...
// ------------------------------------------------------------------
const HERO_IMAGE_URL = ""; // CLOUDINARY: edits/men-hero.jpg
const HERO_VIDEO_URL = "https://res.cloudinary.com/z0afpk9x/video/upload/v1788273094/men_vid_2_enhanced_2560x1440.mp4"; // CLOUDINARY (optional): edits/men-hero.mp4
...
export default function Men() {
    ...
    async function loadData() {
      try {
        const [catRes, prodRes] = await Promise.all([
          ...
          productsAPI.getAll({ gender: "men" }),
        ]);
        if (!cancelled) {
          // Show only Men and Unisex category cards on this page
          const sectionCats = (catRes.data.categories || []).filter(
            (c) => !c.gender || c.gender === "men" || c.gender === "unisex"
          );
  ...
  const filteredProducts = useMemo(() => {
    ...
    // Guarantee no ladies' pieces ever render on this page
    result = result.filter((p) => !p.gender || p.gender === "men" || p.gender === "unisex");
    return result;
  ...
  return (
    <>
      <SEO
        title="The Men's Edit | Zealc.ollection"
        description="The men's collection - timepieces, accessories and refined apparel, considered for every day."
      />
      ...
      >
        ...
        <section className="edit-hero relative bg-onyx overflow-hidden">
          ...
          <div className="max-w-[1440px] mx-auto px-4 sm:px-6 lg:px-10 pt-[120px] md:pt-[210px] pb-14 md:pb-20 relative">
            ...
            >
              <Link to="/shop?gender=men" className="btn-gold inline-flex items-center gap-3">
                Shop the edit
                <ArrowRight size={14} />
              </Link>
              <Link
          ...
          {loading ? (
            ...
            >
              ...
              <p className="text-onyx/60 font-body text-sm max-w-md mx-auto">
                The men's edit is being prepared with care. New pieces arrive
